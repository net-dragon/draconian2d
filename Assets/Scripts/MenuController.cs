...
public class MenuController : MonoBehaviour {
    ...
    //add all pages that can be transitioned to from the menu
    ...
    //the following menu is only required to load the first menu page
    void SetCurrentPage(GameObject pages)
    {
        GameObject p = Instantiate(pages as GameObject);
    ...
    void RevealPageInUI(int index)
    {
        ...
        //start transitioning the current page - this will activate the "out" transition
        t.StartTransition();
        //set the current page to the page that is coming in
